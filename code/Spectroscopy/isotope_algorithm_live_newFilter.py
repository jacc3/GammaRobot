...
host = 'your_detector_ip_address'  # Replace with your detector's IP address
port = 8082
...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
    ...
    # Open a file for writing N42.42 data
    with open('live_n42_data.txt', 'w') as file:
        print("File created. Waiting for N42.42 data...")

        # Receive and write N42.42 messages continuously
        while True:
            data = s.recv(1024)  # Adjust buffer size as needed
            if not data:
                break  # Break if no more data received
            file.write(data.decode('utf-8'))
            file.flush()  # Flush buffer to ensure data is written immediately

print("Connection closed. File closed.")
'''
...
#----------------------------Alteration------------------ same running
    ''''
    
    [peaks, peaks_dict] = find_peaks(spect.counts,
                                 prominence=spect.prom * 0.5,  # prominence threshold
                                 width=(1, None),              # peaks of any width
                                 rel_height=0.5,               # relative height
                                 height=spect.height_vec,      # Use height threshold if available
                                 distance=10)    
                                 
    '''
#-----------------------------Alteration------------------ run twice
'''
detected_peaks = []

# First iteration of peak detection
[peaks, peaks_dict] = find_peaks(spect.counts,
                                 prominence=spect.prom * 0.5,
                                 width=(1, None),
                                 rel_height=0.5,
                                 height=spect.height_vec,
                                 distance=10)

# Store the indices or positions of the detected peaks
detected_peaks.extend(peaks)

# Exclude the detected peaks from subsequent iterations
new_counts = [counts[i] for i in range(len(counts)) if i not in detected_peaks]

# Second iter
[new_peaks, new_peaks_dict] = find_peaks(new_counts,
                                          prominence=spect.prom * 0.5,
                                          width=(1, None),
                                          rel_height=0.5,
                                          height=spect.height_vec,
...
all_peaks = detected_peaks + [peak + len(detected_peaks) for peak in new_peaks]
...
#isotopes_xray_path = "/C:/Users/giusf/OneDrive/Desktop/492/XRay/isotopes_xray.txt"
#file_path = "/C:/Users/giusf/OneDrive/Desktop/492/N42/Cs137_march31_test2_detector.n42"


isotopes_xray_path = "C:/Users/aaron/Dropbox (University of Michigan)/NERS 491_492/Code/Spectroscopy/isotopes_xray.txt"
###
file_path = "C:/Users/aaron/Dropbox (University of Michigan)/NERS 491_492/Code/Spectroscopy/N42/Cs-137_2024-04-01.n42"
