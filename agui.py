...
def calculate_dose(counts, energy, dead_time, dose_limit=5000):
    if len(counts) != len(energy):
        raise ValueError("Counts and energies lists must have the same length.")
    
    energies = energy/(6.242*10**12)
    corrected_counts = [c / (1 - dead_time) for c in counts]
    total_d = sum(c * e for c, e in zip(corrected_counts, energies))
    
    density = 5.76
    volume = 19
    pv = np.multiply(5.76, 19)
    total_dose = np.divide(total_d, pv) * 100000
    percentage_limit = (total_dose / dose_limit) * 100
    
    status = "Safe" if total_dose < dose_limit else "WARNING! Emergency Radiation Action Required"
    
    return {
        "Corrected Dose (mrem)": total_dose,
        "Percentage of Dose Limit": percentage_limit,
        "Status": status
    }

def parse_n42(file_path, peaks):
    tree = ET.parse(file_path)
    root = tree.getroot()
    namespaces = {'n42': 'http://physics.nist.gov/N42/2011/N42'}
    channel_data_element = root.find(".//n42:ChannelData", namespaces)
    
    if channel_data_element is None:
        raise ValueError("ChannelData not found in the n42 file.")
    
    counts = np.array(list(map(int, channel_data_element.text.split())))
...
def czt_efficiency(energy, eta0=1.0, alpha=0.1):
    ...
    - eta0: Efficiency at zero energy (default is 1.0).
    - alpha: Constant for efficiency decay (default is 0.001).
    
    Returns:
    - efficiency: Efficiency of the detector at the given energy.
    """
    return eta0 * np.exp(-alpha * energy)

def calculate_activity(counts, energies, live_time):
    """
    Calculate the activity from the counts, energies, and live time, accounting for detector efficiency.
    
    Parameters:
    - counts: List or array of counts from the spectrum.
    - energies: List or array of energies corresponding to the counts (in keV).
    - live_time: Live time in seconds.
    
    Returns:
    - activity: Corrected activity in Becquerels (Bq).
    """
    total_counts = sum(counts)
    efficiency = np.mean([czt_efficiency(energy) for energy in energies])  # Average efficiency over the energies
    activity = total_counts / (live_time * efficiency)
    return activity
    ...
    def process_file(self):
        if hasattr(self, 'n42_file_path') and ".n42" in self.n42_file_path and os.path.exists(self.n42_file_path):
            ...
            # Calculate dose
                ...
                counts_at_peaks = parse_n42(self.n42_file_path, energies)
                counts_values_array = np.array(list(counts_at_peaks.values()))

                tree = ET.parse(self.n42_file_path)
                root = tree.getroot()
                namespaces = {'n42': 'http://physics.nist.gov/N42/2011/N42'}
                live_time = root.find(".//n42:LiveTimeDuration", namespaces)
                real_time = root.find(".//n42:RealTimeDuration", namespaces)
                match1 = re.search(r"PT([\d\.]+)S", live_time.text)
                match2 = re.search(r"PT([\d\.]+)S", real_time.text)
                live_time = float(match1.group(1))
                real_time = float(match2.group(1)) 
                dead_time = 1-(live_time/real_time)

                dose_result = calculate_dose(counts_values_array, energies, dead_time)
                dose_text = f"\nDose Calculation Results:\n"
                dose_text += f"Corrected Dose (mrem): {dose_result['Corrected Dose (mrem)']:.7f}\n"
